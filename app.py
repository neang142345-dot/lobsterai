...
def send_document(chat_id: int, file_path: str, caption: str = "文件已生成") -> Dict[str, Any]:
    if not TELEGRAM_API:
        raise RuntimeError("BOT_TOKEN not configured")
...
def extract_custom_filename(text: str) -> Tuple[Optional[str], str]:
    ...
    for line in lines:
        ...
        if custom_name is None and (stripped.startswith("文件名:") or stripped.startswith("文件名：")):
            if ":" in stripped:
                custom_name = stripped.split(":", 1)[1].strip()
            else:
                custom_name = stripped.split("：", 1)[1].strip()
        else:
...
def extract_effective_request(text: str, reply_to_message: dict) -> str:
    ...
    command_like = any(
        kw in current_text.lower()
        for kw in ["pdf", "docx", "word", "xlsx", "excel", "ppt", "pptx", "生成", "做好", "发我", "整理"]
    )
...
def get_theme_palette(theme: Optional[str]):
...
def choose_subtitle_by_style(style: Optional[str]) -> str:
    if style == "汇报":
        return "管理层汇报材料"
    if style == "商务":
        return "商务演示文稿"
...
def fuse_search_results(search_data: Dict[str, Any], max_items: int = 5) -> Dict[str, Any]:
    ...
    return {"summary_text": "\n".join(summaries), "sources": sources}
...
def detect_intent_and_file_type_fallback(text: str) -> Dict[str, str]:
    raw = text or ""
    t = raw.lower()
...
def enhance_content(intent: str, content: str, style: Optional[str] = None) -> str:
    clean = content.strip() if content and content.strip() else "（空内容）"
    lines = [line.strip() for line in clean.splitlines() if line.strip()]

    if intent == "application" and len(lines) <= 3:
        clean = (
            "申请书\n\n"
            "尊敬的领导：\n"
    ...
    elif intent == "contract" and len(lines) <= 5:
        clean = (
            "合作协议\n\n"
            "甲方：\n"
            "乙方：\n"
...
def normalize_ai_result(data: dict, fallback: Optional[dict] = None) -> dict:
...
12. 如果 file_type 是 pptx：尽量拆成 4 到 6 页；每页只保留核心信息；每页最多 4 个 bullet；bullet 必须是短句。
...
def create_generation_result(payload: Dict[str, Any], task_id: str) -> Dict[str, Any]:
    ...
    if need_search:
        try:
            ...
            fused_search = fuse_search_results(
                search_data,
                max_items=int(payload.get("max_results", DEFAULT_MAX_RESULTS)),
            )
...
def wrap_text_lines(text: str, c, font_name: str, font_size: int, max_width: float) -> List[str]:
...
def create_pdf(text: str, output_path: str, style: Optional[str] = None, theme: Optional[str] = None) -> None:
...
def add_doc_divider(doc: Document) -> None:
    p = doc.add_paragraph()
    run = p.add_run("—" * 42)
...
def create_docx(text: str, output_path: str, style: Optional[str] = None, theme: Optional[str] = None) -> None:
    palette = get_theme_palette(theme)
    doc = Document()
    title = build_title(text)
...
def style_excel_sheet(ws, style: Optional[str] = None, theme: Optional[str] = None) -> None:
    palette = get_theme_palette(theme)
...
def create_xlsx(text: str, output_path: str, style: Optional[str] = None, theme: Optional[str] = None) -> None:
    wb = Workbook()
    ws = wb.active
    ws.title = "Sheet1"
...
def create_slide_header(slide, title_text: str, palette: Dict[str, Any]) -> None:
...
def create_pptx_structured(
    ...
    for item in slides_data:
        ...
        p_footer.text = choose_subtitle_by_style(style)
...
def create_pptx(text: str, output_path: str, style: Optional[str] = None, theme: Optional[str] = None) -> None:
    title = build_title(text)
    lines = [line.strip() for line in text.splitlines() if line.strip()]
...
def set_webhook():
    ...
    if not BOT_TOKEN or not url:
        return {"ok": False, "error": "missing BOT_TOKEN or RENDER_EXTERNAL_URL"}, 400
